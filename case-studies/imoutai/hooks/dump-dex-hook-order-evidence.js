...
`;
  ...
  // _build/entry-order-evidence.js
  var require_entry_order_evidence = __commonJS({
    "_build/entry-order-evidence.js"() {
      init_frida_java_bridge();
      function sendEvent(data) {
        send({ type: "order-evidence", data });
      }
      ...
      function bodySummary(request) {
        return safe(function() {
          ...
          const Buffer2 = frida_java_bridge_default.use("okio.Buffer");
      ...
      function enumerateCaptchaSdk() {
        safe(function() {
          const names = frida_java_bridge_default.enumerateLoadedClassesSync().filter(function(name) {
            return name.indexOf("com.netease.nis.captcha") === 0;
          }).sort();
          const classes = names.map(function(name) {
            return safe(function() {
              const cls = frida_java_bridge_default.use(name).class;
              const methods = [];
              const declared = cls.getDeclaredMethods();
              for (let i = 0; i < declared.length; i++) methods.push(String(declared[i]));
              return { name, methods: methods.sort() };
            }, "captcha-class") || { name, methods: [] };
          });
          sendEvent({ kind: "captcha-sdk-inventory", classCount: classes.length, classes });
        }, "captcha-inventory");
      }
      ...
      function install() {
        frida_java_bridge_default.perform(function() {
          safe(function() {
            const a = frida_java_bridge_default.use("com.moutai.mall.api.a");
            a.intercept.implementation = function(chain) {
              ...
              if (kind) sendEvent({ kind: "http-response", phase: kind, path, response: responseSummary(response) });
              return response;
            };
          }, "api-intercept");
          enumerateCaptchaSdk();
      ...
      rpc.exports = {
        getStats: function() {
          return { ok: true, agent: "order-evidence", readOnly: true };
        }
      };
    }
  });
  require_entry_order_evidence();
})();
