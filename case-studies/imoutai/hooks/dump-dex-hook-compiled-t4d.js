    ...
    "android/case-studies/imoutai/hooks/dump-dex-hook.js"() {
      ...
      (function() {
        ...
        function attach_onload(address, source) {
          ...
          var key = "onload:" + address.toString();
          if (hooked[key]) return;
          hooked[key] = true;
          ...
          try {
            Interceptor.attach(address, {
              onEnter: function(args) {
                log("A5: JNI_OnLoad enter @" + address + " source=" + source);
                install_findclass(args[0]);
              }
            });
            log("A5: JNI_OnLoad observer attached @" + address + " source=" + source);
          } catch (e) {
          ...
          try {
            Interceptor.attach(addr, {
              onEnter: function(args) {
                ...
                log("A2: " + name + (sig === null ? "" : "(" + sig + ")") + " caller=" + callerName + (this.block ? " [BLOCKED]" : ""));
                if (name === "abort" || name === "__stack_chk_fail") {
                  try {
                    var bt = Thread.backtrace(this.context, Backtracer.ACCURATE).slice(0, 12).map(DebugSymbol.fromAddress).join("\n");
