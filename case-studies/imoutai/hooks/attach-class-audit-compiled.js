...
`;
  ...
  var kDefaultConstructorDebugInfo = BufferShim.from([3, 0, 7, 14, 0]);
  var kDalvikAnnotationTypeThrows = "Ldalvik/annotation/Throws;";
  var kNullTerminator = BufferShim.from([0]);
  function mkdex(spec) {
  ...
  function audit_loaded_classes() {
    ...
    var run = function() {
      try {
        ...
        frida_java_bridge_default.enumerateClassLoadersSync().forEach(function(loader) {
          try {
            result.loaders.push({
    ...
    try {
      if (frida_java_bridge_default.performNow) frida_java_bridge_default.performNow(run);
      else frida_java_bridge_default.perform(run);
    } catch (e) {
      result.error = String(e);
    }
    emit({ type: "class_audit", result });
    return result;
  }
  rpc.exports = {
    audit: function() {
