...
`;
  ...
  // entry-annot2.js
  var require_entry_annot2 = __commonJS({
    "entry-annot2.js"() {
      init_frida_java_bridge();
      var PKG_PREFIX = "com.moutai.mall";
      ...
      function dumpClass(cls) {
        ...
        for (let j = 0; j < declared.length; j++) {
          ...
          try {
            item.annotations = annStrings(m.getDeclaredAnnotations());
          } catch (e) {
          ...
          try {
            ...
            item.parameterAnnotations.push(["(paramann failed: " + e + ")"]);
          }
          methods.push(item);
        }
        const fields = [];
        try {
          ...
          for (let j = 0; j < fs.length; j++) {
            ...
            try {
              item.annotations = annStrings(f.getDeclaredAnnotations());
            } catch (e) {
              item.annotations.push("(ann failed: " + e + ")");
            }
            fields.push(item);
