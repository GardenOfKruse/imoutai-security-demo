...
`;
  ...
  var SHELL_SOS, stats, live_emitted, live_scanning, live_scan_enabled;
  var init_dump_dex_hook = __esm({
    "android/case-studies/imoutai/hooks/dump-dex-hook.js"() {
      init_frida_java_bridge();
      SHELL_SOS = ["libDexHelper.so", "libdexvmp.so", "libSecShell", "libbangcle_risk.so", "libRiskStub.so"];
      stats = { pthread_total: 0, pthread_suppressed: 0, modules: {}, dex_captured: 0, exit_blocked: 0, native_signal_blocked: 0 };
      (function() {
          ...
          try {
            Interceptor.attach(addr, {
              onEnter: function(args) {
                ...
                this.block = (sig === 19 || sig === 33) && (callerName === "libDexHelper.so" || callerName === "libdexvmp.so" || callerName.indexOf("libSecShell") !== -1 || callerName === "libbangcle_risk.so");
                log("A2: " + name + (sig === null ? "" : "(" + sig + ")") + " caller=" + callerName + (this.block ? " [candidate-block]" : ""));
              },
              onLeave: function(retval) {
                if (this.block) {
                  retval.replace(0);
                  stats.native_signal_blocked++;
                  log("A2: targeted native signal return blocked #" + stats.native_signal_blocked);
                }
              }
            });
          } catch (e) {
