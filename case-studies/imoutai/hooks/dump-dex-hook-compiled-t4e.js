...
`;
  ...
  function find_app_loader() {
    ...
    try {
      frida_java_bridge_default.enumerateClassLoadersSync().forEach(function(l) {
        if (app_loader) return;
        try {
          var cn = l.getClass().getName();
          if (cn !== "dalvik.system.PathClassLoader") return;
          try {
            l.loadClass("com.secneo.apkwrapper.H");
            app_loader = l;
          } catch (e) {
            if (!app_loader) app_loader = l;
          }
        } catch (e) {
        }
      });
    ...
    if (app_loader) log("B2: app loader = " + app_loader.getClass().getName() + " @ " + app_loader.hashCode());
    return app_loader;
  }
  ...
  var SHELL_SOS, stats, native_redirect_path, app_loader, maps_filter_enabled, maps_filter_fds, maps_filter_streams, attach_jni_onload_observer, live_emitted, live_scanning, live_scan_enabled;
  var init_dump_dex_hook = __esm({
    "android/case-studies/imoutai/hooks/dump-dex-hook.js"() {
      ...
      (function() {
        ...
        if (!dlsym) {
          log("A5: dlsym not found");
          return;
        }
        var hooked = {};
        function install_findclass(env) {
          try {
            ...
            var originalFn = new NativeFunction(original, "pointer", ["pointer", "pointer"]);
            var observer = new NativeCallback(function(e, namePtr) {
              stats.jni_findclass_calls++;
              var name = "";
              try {
              ...
              var ret = originalFn(e, namePtr);
              if (ret.isNull()) stats.jni_findclass_null++;
              log("A5: FindClass(" + name + ") -> " + (ret.isNull() ? "NULL" : "ok"));
              return ret;
            }, "pointer", ["pointer", "pointer"]);
            var clone = Memory.alloc(Process.pointerSize * 240);
            Memory.copy(clone, table, Process.pointerSize * 240);
            clone.add(Process.pointerSize * 6).writePointer(observer);
            env.writePointer(clone);
            hooked[key] = true;
            log("A5: JNIEnv FindClass observer installed original=" + key);
          } catch (e) {
            log("A5: FindClass observer fail: " + e);
          }
        }
        function attach_onload(address, source) {
          if (!address || address.isNull()) return;
          var stableAddress = ptr(address.toString());
          var key = "onload:" + stableAddress.toString();
          if (hooked[key]) return;
          hooked[key] = true;
          stats.jni_onload_observed++;
          try {
            Interceptor.attach(stableAddress, {
              onEnter: function(args) {
                log("A5: JNI_OnLoad enter @" + stableAddress + " source=" + source);
                install_findclass(args[0]);
              }
            });
            log("A5: JNI_OnLoad observer attached @" + stableAddress + " source=" + source);
          } catch (e) {
              ...
              try {
                ...
                for (var i = 0; i < exports.length; i++) {
                  if (exports[i].name === "JNI_OnLoad") {
                    ...
                    attach_onload(exports[i].address, "module-observer");
                    break;
                  }
                }
              } catch (e) {
