...
`;
  ...
  function dump_loaded_reflection() {
    ...
    try {
      var names = frida_java_bridge_default.enumerateLoadedClassesSync().filter(function(n) {
        return n.indexOf("com.moutai.mall") === 0;
      }).sort();
      var emitted = 0;
      for (var i = 0; i < names.length; i++) {
        ...
        var methods = [];
        try {
          var cls = frida_java_bridge_default.use(name).class;
          ...
          for (var j = 0; j < declared.length; j++) {
            ...
            try {
              ...
              item.annotations.push("(annotations failed: " + e1 + ")");
            }
            try {
              ...
              item.parameterAnnotations.push(["(parameter annotations failed: " + e2 + ")"]);
            }
            methods.push(item);
          }
        } catch (e) {
          methods.push("(reflection failed: " + e + ")");
        }
        send({ type: "reflection", className: name, methods });
        emitted++;
      }
      log("E3: reflection metadata emitted classes=" + emitted);
    } catch (e) {
      log("E3: reflection dump failed: " + e);
    }
  }
  function install_java_hooks() {
    ...
    if (MP34_REFLECTION_DUMP) {
      setTimeout(function() {
        try {
          frida_java_bridge_default.perform(dump_loaded_reflection);
        } catch (e) {
          log("E3: deferred reflection failed: " + e);
        }
      }, 350);
    }
  }
