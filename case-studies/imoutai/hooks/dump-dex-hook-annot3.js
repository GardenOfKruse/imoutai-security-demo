...
`;
  ...
  // entry-annot3.js
  var require_entry_annot3 = __commonJS({
    "entry-annot3.js"() {
      ...
      function safe(fn, fallback) {
        try {
          ...
          return fallback === void 0 ? "(err: " + e + ")" : fallback;
        }
      }
      function dumpEndpoints() {
        frida_java_bridge_default.perform(function() {
          const out = {};
          safe(function() {
            const ep = frida_java_bridge_default.use("com.moutai.mall.env.Endpoints");
            const vals = ep.values();
            for (let i = 0; i < vals.length; i++) {
              const v = vals[i];
              out[String(v.name())] = safe(function() {
                return String(v.getApp());
              });
            }
          });
      ...
      function findOkioBuffer() {
        const names = frida_java_bridge_default.enumerateLoadedClassesSync().filter(function(n) {
          return n.indexOf("okio.") === 0;
        });
        for (let i = 0; i < names.length; i++) {
          try {
            const c = frida_java_bridge_default.use(names[i]).class;
            ...
            if (hasUtf8 && hasWrite) {
              const inst = frida_java_bridge_default.use(names[i]).$new();
              return names[i];
            }
          } catch (e) {
          }
      ...
      function install() {
        frida_java_bridge_default.perform(function() {
          ...
          modelCtors.forEach(function(cn) {
            safe(function() {
              const cls = frida_java_bridge_default.use(cn).class;
              const ctors = cls.getDeclaredConstructors();
              for (let i = 0; i < ctors.length; i++) {
                (function(ctor) {
                  const k = cls.getDeclaredFields().length;
                  const wrapper = frida_java_bridge_default.use(cn);
          ...
          safe(function() {
            const ab = frida_java_bridge_default.use("com.moutai.mall.api.a$b");
              ...
              try {
                ...
                if (body !== null && bufClsName) {
                  bodyStr = safe(function() {
                    const buf = frida_java_bridge_default.use(bufClsName).$new();
                    body.writeTo(buf);
