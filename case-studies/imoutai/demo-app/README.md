...
> **⚠️ 红线**：支付链接只做拼接展示，禁止真实调用；实弹模式仅在授权窗口内的单轮 runbook 执行。
...
| 🔴 实弹模式（Live） | **全真实环境**：真实请求、真实业务数据、真实业务步骤（真实登录、真实下单**止步支付**），请求中不含任何模拟标记 | 仅限授权 runbook F 组用例的单轮执行 |

实弹模式有四重硬门禁（`src/lib/realApi.js`）：授权三确认 → 时段校验（允许窗口 20:00–01:00 / 07:00–18:00；**高峰 06:00–06:15 申购期绝对禁发**；缓冲时段需对接人知情）→ 设备 HeaderMap 档案（自动生成，真实 App 形态无模拟标记）→ 请求预算 ≤300。任一不满足即拒绝发请求。
