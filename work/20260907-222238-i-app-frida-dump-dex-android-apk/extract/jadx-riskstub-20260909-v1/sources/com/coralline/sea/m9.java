package com.coralline.sea;

/* JADX INFO: loaded from: E:\code\逆向\android\work\20260907-222238-i-app-frida-dump-dex-android-apk\extract\runtime-code-files-25103-20260909-raw\RiskStub-a.dex */
public class m9 {
    public static final int A = 2;
    public static final int B = 64;
    public static final int C = 128;
    public static final int D = 256;
    public static final int E = 512;
    public static final int F = 1024;
    public static final int G = 2048;
    public static final int H = 4096;
    public static final int I = 8192;
    public static final int J = 16384;
    public static final int K = 32768;
    public static final int L = 65536;
    public static final int M = 131072;
    public static final int N = 262144;
    public static final int O = 524288;
    public static final int a = 61440;
    public static final int b = 49152;
    public static final int c = 40960;
    public static final int d = 32768;
    public static final int e = 24576;
    public static final int f = 16384;
    public static final int g = 8192;
    public static final int h = 4096;
    public static final int i = 2048;
    public static final int j = 1024;
    public static final int k = 512;
    public static final int l = 448;
    public static final int m = 256;
    public static final int n = 128;
    public static final int o = 64;
    public static final int p = 56;
    public static final int q = 32;
    public static final int r = 16;
    public static final int s = 8;
    public static final int t = 7;
    public static final int u = 4;
    public static final int v = 2;
    public static final int w = 1;
    public static final int x = 3;
    public static final int y = 0;
    public static final int z = 1;

    public static final int a(int i2) {
        return (i2 & a) == 24576 ? 0 : 1;
    }

    public static int a(int i2, byte[] bArr, int i3) {
        return i6.c(i2, bArr, i3);
    }

    public static int a(String str, int i2, int i3) {
        return i6.b(str, i2, i3);
    }

    public static boolean a(String str) {
        return i6.f(str);
    }

    public static final int b(int i2) {
        return (i2 & a) == 8192 ? 0 : 1;
    }

    public static int b(int i2, byte[] bArr, int i3) {
        return i6.d(i2, bArr, i3);
    }

    public static final int c(int i2) {
        return (i2 & a) == 16384 ? 0 : 1;
    }

    public static final int d(int i2) {
        return (i2 & a) == 4096 ? 0 : 1;
    }

    public static final int e(int i2) {
        return (i2 & a) == 40960 ? 0 : 1;
    }

    public static final int f(int i2) {
        return (i2 & a) == 32768 ? 0 : 1;
    }

    public static final int g(int i2) {
        return (i2 & a) == 49152 ? 0 : 1;
    }

    public static int h(int i2) {
        return i6.f(i2);
    }
}
