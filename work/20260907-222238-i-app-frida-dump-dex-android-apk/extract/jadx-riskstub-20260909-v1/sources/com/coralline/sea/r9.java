package com.coralline.sea;

import android.os.Process;
import java.util.HashMap;

/* JADX INFO: loaded from: E:\code\逆向\android\work\20260907-222238-i-app-frida-dump-dex-android-apk\extract\runtime-code-files-25103-20260909-raw\RiskStub-a.dex */
public class r9 {
    public static r9 a = null;
    public static final String b = "sys_process_info";

    public static class a {
        public int a;
        public String b;
        public String c;
        public String d;
        public String e;
        public String f;
        public String g;

        public a(int i, String str, String str2, String str3, String str4) {
            this.a = i;
            this.b = str;
            this.e = str2;
            this.f = str3;
            this.g = str4;
        }

        public String a() {
            return this.f;
        }

        public void a(int i) {
            this.a = i;
        }

        public void a(String str) {
            this.f = str;
        }

        public String b() {
            return this.g;
        }

        public void b(String str) {
            this.g = str;
        }

        public int c() {
            return this.a;
        }

        public void c(String str) {
            this.e = str;
        }

        public String d() {
            return this.e;
        }

        public void d(String str) {
            this.b = str;
        }

        public String e() {
            return this.b;
        }

        public void e(String str) {
            this.c = str;
        }

        public String f() {
            return this.c;
        }

        public void f(String str) {
            this.d = str;
        }

        public String g() {
            return this.d;
        }

        public String toString() {
            return "Info{pid=" + this.a + ", pname='" + this.b + "', reason='" + this.c + "', uname='" + this.d + "', pkg_name='" + this.e + "', app_name='" + this.f + "', md5='" + this.g + "'}";
        }
    }

    public static class b {
        public String a;
        public int b;
        public int c;
        public int d;
        public String e;
        public String f;

        public b(String str) {
            String[] strArrSplit = str.split("\\s+");
            if (strArrSplit.length >= 4) {
                String str2 = strArrSplit[strArrSplit.length - 1];
                String strReplace = (str2.contains(":") ? str2.split(":")[0] : str2).replace("/", "@#@");
                if (strReplace.contains(".")) {
                    String str3 = strArrSplit[0];
                    this.a = str3;
                    this.b = Process.getUidForName(str3);
                    this.c = Integer.parseInt(strArrSplit[1]);
                    this.d = Integer.parseInt(strArrSplit[2]);
                    this.f = strArrSplit[strArrSplit.length - 1];
                    for (String str4 : strReplace.split("@#@")) {
                        if (str4.contains(".")) {
                            this.e = str4;
                            return;
                        }
                    }
                }
            }
        }

        public String toString() {
            return String.format(this.f, this.e, this.a);
        }
    }

    public static synchronized r9 a() {
        if (a == null) {
            a = new r9();
        }
        return a;
    }

    public HashMap<String, b> a(Boolean bool) {
        b bVar;
        HashMap<String, b> map = new HashMap<>();
        String strA = b8.a();
        if (strA == null) {
            return map;
        }
        for (String str : strA.split("\n")) {
            try {
                if (!bool.booleanValue()) {
                    bVar = new b(str);
                    String str2 = bVar.e;
                    if (str2 == null || (!str2.startsWith("com.android.") && !bVar.e.startsWith("com.mi.") && !bVar.e.startsWith("com.huawei.") && !bVar.e.startsWith("com.sec.android."))) {
                        String str3 = bVar.e;
                        map.put(str3, bVar);
                    }
                } else if (str.split("\\s+")[0].equals("root") && (str3 = (bVar = new b(str)).e) != null) {
                    map.put(str3, bVar);
                }
            } catch (Exception e) {
            }
        }
        return map;
    }
}
