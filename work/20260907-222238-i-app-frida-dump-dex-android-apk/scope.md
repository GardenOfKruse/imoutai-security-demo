...
- evidence_of_auth: |
    ...
    硬性约束（客户最新确认，2026-09-12，等同 scope 条款）：
    1. 除 06:00–06:15 客户维护窗口外，其他时间均可执行本演示项目测试；
    2. 允许多次低频执行；禁止大并发、高频调用和超预算执行；
    3. 当前验证阶段执行收紧为 1 个工作进程，禁止并发；
...
- notes: |
    ...
    时间规则（客户最新确认 2026-09-12）：除 06:00–06:15 客户维护窗口外，其余时间均允许执行；06:00–06:15 禁止任何生产请求。
    执行模式：可多次低频执行；单进程、速率与总量受 runbook 硬门禁约束（AGENTS.md）。
    其余分析保持本地离线。
